...
describe("Notification tests", () => {
  ...
  it("renders correct list items", () => {
    ...
    wrapper.find("ul").forEach((node, idx) => {
      expect(node.equals(<NotificationItem type={listNotifications[idx].type} value={listNotifications[idx].value} html={listNotifications[idx].html} key={listNotifications[idx].id} markAsRead={wrapper.instance().markAsRead} id={listNotifications[idx].id} />));
    });
  });
  ...
  it("renders correctly when listCourses is not passed", () => {
    const wrapper = shallow(<Notifications displayDrawer={true} />);
    expect(wrapper.containsMatchingElement(<NotificationItem type="default" value="No new notification for now" />)).toBe(true);
  });
